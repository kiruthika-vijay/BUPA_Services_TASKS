...
export const apiClient = axios.create({
    baseURL: 'https://localhost:7009/api', // Replace with your API URL
    headers: {
    ...
    (error) => {
        if (error.response && error.response.status === 401) {
            ...
            if (currentPath.startsWith('/employees')) {
                window.location.href = '/employees/login'; // Redirect to customer login
            }
            else {
                    window.location.href = '/employees'; // Default login page if no specific path
            }
        }
        return Promise.reject(error);
    }
